...
% consider the clamped-clamped von Kármán beam discussed in [1].
% 
% 
% 
...
numberOuterModes = (n-sSSMDim)/2;
Vo = V(:,sSSMDim+[1:2*numberOuterModes]); 
Wo = W(sSSMDim+[1:2*numberOuterModes],:); Lo = Wo*A*Vo;
% Forcing vector
...
% Maximal amplitude response at which the rom is trusted
ampMax = amplitudeCal(end);
[~,pos] = min(abs(BBCInfo.amplitude-0.011));
omegaSpan = abs(imag(lambda(1)))*[0.9 1.3]; % frequency span

% Compute FRCs with SSMLearn
FRCData = analyticalFRCf(IMInfoF, RDInfoF, forcingAmplitude, amplitudeFunction,BBCInfo.amplitudeNormalForm(pos)*1.1);
    
% Compute FRCs with SSMTool
%FRCSSMTool = SSMToolFRCcustom(M, C, K, fnl, loadVector, forcingAmplitude, outdof, omegaSpan, ROMOrder,2e3);
...
% Here we simulate a forced periodic solution predicted by the ROM on the 
% full system.

iForcingAmplitude = 1; % index for the desired forcing amplitude in the vector forcingAmplitude
FRCDataCurr = FRCData.([ 'F' num2str(iForcingAmplitude)]);
resonanceBeforeAfter = 1; % 1: before, 2: after
FRCDataCurrFreq = FRCDataCurr.Freq(resonanceBeforeAfter,:);
FRCDataCurrZIC = FRCDataCurr.z_IC(resonanceBeforeAfter,:);

% Initial condition and parameters
iForcingFrequency = 98; % distance to resonance in percentage
iOmega = floor( iForcingFrequency/101 * length(FRCDataCurrFreq)) + 1;
fOmegaTest = FRCDataCurrFreq(iOmega); 
z0 = [FRCDataCurrZIC(iOmega); conj(FRCDataCurrZIC(iOmega))];
...
plot(tTestF,yTest(1,:),'Linewidth',2,'DisplayName','FOM');
plot(tTest,yTestRec(1,:),':','Linewidth',2,'DisplayName','SSMLearn ROM');
legend
xlabel('time')
ylabel('modal displacement')
subplot(212) 
plot(tTestF,yTest(2,:),'Linewidth',2,'DisplayName','FOM');
plot(tTest,yTestRec(2,:),':','Linewidth',2,'DisplayName','SSMLearn ROM');
xlabel('time')
ylabel('modal velocity')

customFigure('subPlot',[2 1]); subplot(211) 
indPlot = 54;
plot(tTestF,xTest(indPlot,:),'Linewidth',2,'DisplayName','FOM');
plot(tTest,xTestRec(indPlot,:),':','Linewidth',2,'DisplayName','SSMLearn ROM');
% plot(tTestF,ampTest,'Linewidth',2,'DisplayName','FOM');
% plot(tTest,ampTestRec,':','Linewidth',2,'DisplayName','SSMLearn ROM');
legend
xlabel('time')
ylabel('response ')
subplot(212) 
plot(tTestF,100 * sqrt(sum((xTest-xTestRec).^2)) / max(sqrt(sum((xTest).^2))),'k','Linewidth',2,'DisplayName','FOM');
xlabel('time')
set(gca,'yscale','log')
ylabel('global error %')
%% Test Reduced & Normal form dynamics
% Here we simulate the transient towards a forced periodic solution 
% predicted by the ROM on the  full system.
...
fOmegaTest = FRCDataCurrFreq(iOmega); 
z0 = zeros(2,1); 
iOmegaIC = floor(iForcingFrequencyIC/101 * length(FRCDataCurrFreq)) + 1;
z0 = [FRCDataCurrZIC(iOmegaIC); conj(FRCDataCurrZIC(iOmegaIC))]
