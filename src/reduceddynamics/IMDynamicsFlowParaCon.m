function RDInfo = IMDynamicsFlowParaCon(xpData,varargin)
% RDInfo = IMDynamicsParaCon(etaData,varargin)
...
% 0 = R(x_o,p_o), then we enforce the regression weights to be
% identified to satisfy 0 = W_r * phi(x_o,p_o) for each of the
...
for ii = 1:size(xpData,1)
    ...
    if size(xpData,2)>2 
        Pi = xpData{ii,3}; P = [P repmat(Pi,1,length(ti))]; end
...
[W_r,phi,Dxphi,Dpphi,Expmat,l_opt,Err] = ...
    RidgeRegressionConstrainedParametric(t,X,P,dXdt,[Xo; Po],zeros(k,size(Xo,2)),options);
if l > 0
