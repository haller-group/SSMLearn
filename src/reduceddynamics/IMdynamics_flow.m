...
%
% OUTPUTS
% R  - vector field in the given coordinates of X_traj
...
switch options.style
    case 'modal'
        ...
        [V,~,~] = eig_sorted(W_r(:,1:k)); iT = @(x) V\x; T = @(y) V*y;
...
RMS = mean(vecnorm((R(X) - dXdt).*L2)) / mean(vecnorm(dXdt.*L2));
...
function [Maps_info_opt]=initialize_nf_flow(V,D,d,W_r,X_traj,options)
...
[~,Expmat_n] = multivariate_polynomial(k,2,options.N_PolyOrd);
% Reshape of trajectories into matrices
...
for ii = 1:size(X_traj,1)
    t_in = X_traj{ii,1}; Y_in = V\(X_traj{ii,2});
